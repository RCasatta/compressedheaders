...
static GENESIS_RAW_HEX: &'static str = "0100000000000000000000000000000000000000\
                                        000000000000000000000000000000003ba3edfd\
                                        7a7b12b27ac72c3e67768f617fc81bc3888a5132\
...
pub struct BlockHeader {
    ...
    // encoded as a a float (with well-defined rounding, of course)
    pub bits: [u8; 4],
...
impl BlockHeader {
    ...
    pub fn hash(&self) -> [u8; 32] {
        ...
        //TODO FIX nonsense passing from string
        let bytes: Vec<u8> = (&sha2b.result_str()).from_hex().unwrap();
...
mod tests {
    ...
    pub fn test_as_compressed_bytes() {
        ...
        let genesis_compressed = "010000003ba3edfd7a7b12b27ac72c3e67768f617fc8\
                                  1bc3888a51323a9fb8aa4b1e5e4a29ab5f491dac2b7c";
        assert_eq!(b.to_hex(), genesis_compressed);
    }
    ...
    pub fn test_block_headers_reconstruct() {
        ...
        let block_143 = "61188712afd4785d18ef15db57fb52dd150b56c8b547fc6bbf23ec4900000000";

        test_block_header_reconstruct(
            test_data_144,
            144,
            String::from(zeroes),
            String::from(genesis_block),
            String::from(block_143),
        );

        let test_data_2016 = include_bytes!("../../examples/2016/0").to_vec();
        let block_2015 = "6397bb6abd4fc521c0d3f6071b5650389f0b4551bc40b4e6b067306900000000";
        test_block_header_reconstruct(
            test_data_2016,
            2016,
            String::from(zeroes),
            String::from(genesis_block),
            String::from(block_2015),
        );

        let test_data_2016_20 = include_bytes!("../../examples/2016/20").to_vec();
        let block_40320 = "45720d24eae33ade0d10397a2e02989edef834701b965a9b161e864500000000";
        let block_40319 = "1a231097b6ab6279c80f24674a2c8ee5b9a848e1d45715ad89b6358100000000";
        test_block_header_reconstruct(
            test_data_2016_20,
            2016,
            String::from(block_40319),
            String::from(block_40320),
            String::from(block_42335),
        );

        let test_data_2016_21 = include_bytes!("../../examples/2016/21").to_vec();
        let block_42336 = "1296ba2f0a66e421d7f51c4596c2ce0820903f3d81a953173778000b00000000";
        let block_44351 = "d55e1b468c22798971272037d6cc04fdac73913c0012d0d7630c2e1a00000000";
        test_block_header_reconstruct(
            test_data_2016_21,
            2016,
            String::from(block_42335),
            String::from(block_42336),
            String::from(block_44351),
        );

        //TODO add test continuity between chunk (prev first hash equal last previous chunk)
    }

    pub fn test_block_header_reconstruct(
        test_data: Vec<u8>,
        chunk_size: u32,
        first_prev_hash: String,
        first_hash_verify: String,
        last_hash_verify: String,
    ) {
        ...
        for i in 0..chunk_size - 1 {
            ...
            let current_as_block: BlockHeader =
                BlockHeader::from_compressed_bytes(compressed_block_bytes, prev_hash, prev_diff);
