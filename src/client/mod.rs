        ...
        match r {
            Ok(block_header_rpc_response) => {
                ...
                if last_block==0 && height%1000==0 {
                    ...
                    synced_height = sync(&mut block_headers_map, &block_headers_bytes, height, synced_height) ;
                }
                ...
                let sleep = match block_hash_option {
                    ...
                    None => {
                        ...
                        last_block = height;
                        block_hash = block_headers_map.get(&(height - 6)).unwrap().hash_be().to_hex();   //going back 144 blocks to support reorgs one day long
...
fn sync( block_headers_map : &mut HashMap<usize,BlockHeader>, block_headers_bytes : &Arc<Mutex<Vec<u8>>>, height : usize, synced_height : usize) -> usize {
    ...
    match sync_to_option {
        Some(sync_to) => {
            ...
            for i in synced_height..sync_to {
                match i % 2016 {
                    0 => block_headers_bytes_lock.extend(block_headers_map.remove(&i).unwrap().as_bytes().into_iter()),
                    _ => block_headers_bytes_lock.extend(block_headers_map.remove(&i).unwrap().as_compressed_bytes().into_iter()),
                }
            }
            sync_to
        },
